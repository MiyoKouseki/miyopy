...
def plotspectrogram(data, samp_rate, per_lap=0.9, wlen=None, log=False,
                outfile=None, fmt=None, axes=None, dbscale=False,
                mult=8, zorder=None, title=None,
                auto_frange=False,
                flim=[1e-11,1e-5],
    ...
    if auto_frange:
        ...
        _range = float(flim[1]-flim[0])
    #print '_range = {0:4.1e}'.format(_range)
    #vmin = specgram.min() + vmin * _range
    #vmax = specgram.min() + vmax * _range
    vmin = 1e-11 + vmin * _range
    vmax = 1e-11 + vmax * _range
    norm = Normalize(vmin, vmax, clip=True)
    norm = LogNorm(vmin, vmax,clip=True)
    if not axes:
        fig = plt.figure(figsize=(6,4),dpi=480)
